...
class YamlWriter(QObject):
    ...
    def run(self):
        try:
            ...
            with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8", dir=dir_, delete=False) as tf:
                tmp = tf.name
                yaml.safe_dump(self.data, tf, sort_keys=False, allow_unicode=True, default_flow_style=False)
