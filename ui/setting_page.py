...
class SettingPage(QWidget):
    ...
    def __init__(self, parent):
        ...
        title = QLabel("Setting Page"); title.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        ...
        body_layout = QVBoxLayout(self.body); body_layout.setContentsMargins(12,12,12,12); body_layout.setSpacing(8)
        ...
        self.save_btn.setStyleSheet("QPushButton { background:#1976D2; color:white; border:none; border-radius:6px; padding:6px 14px; }"
    ...
    def _on_saved(self, path: str):
        ...
        self.parent.toast.show_notice("info", "Config Saved.", "Config Saved. Restart Aim Sys to apply settings.")
    ...
    def _make_settings(self, layout: QFormLayout, args: dict, path: tuple):
        for key, val in args.items():
            ...
            if isinstance(val, dict) and not {"value","options"} <= set(val.keys()):
                box = QGroupBox(key); box.setStyleSheet("QGroupBox{color:#ddd; font-weight:600;}")
                inner = QFormLayout(box); inner.setSpacing(8)
                layout.addRow(box)
                self._make_settings(inner, val, p)
                continue

            label = QLabel(key); label.setStyleSheet(_TEXT_STYLE)
            ...
            if isinstance(val, bool):
                ...
                w = QSpinBox(); w.setRange(min(-10**9, val-10**6), max(10**9, val+10**6))
                w.setValue(val)#; w.valueChanged.connect(lambda *_: self.valueChanged.emit(self.values()))
                self._widgets[p] = w
                layout.addRow(label, w)
            elif isinstance(val, float):
                w = QDoubleSpinBox(); w.setDecimals(6); w.setSingleStep(0.1)
    ...
    def _get_value(self, w):
        ...
        if isinstance(w, QSpinBox):       return int(w.value())
        if isinstance(w, QDoubleSpinBox): return float(w.value())
