...
class Updater():
    def __init__(self, install_dir: str | Path | None = None, on_update_func: callable | None = None):
        ...
        with requests.get(url, stream=True, timeout=60) as r:
            ...
            with open(dst, "wb") as f:
                for chunk in r.iter_content(chunk_size=8192):
                    ...
                    if mb != last_print:
                        last_print = mb
                        self.on_update("update", f"Downloading... {mb:.2f} MB",)
                        print(
    ...
    def start_update(self) -> bool:
        """
        回傳 True 表示已安裝新版本（應該重啟 APP）
        回傳 False 表示不需要更新或失敗
        """
        info = self.get_latest()
        tag_name = info["tag_name"]     
        pkg_name = tag_name + "-pkg.zip"      # 例如 "TR-0.1.0"
        assets = info["assets"]
        ...
        if ver_asset is None:
            print("no version.json asset found")
            return False
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not self._is_newer(remote_ver, self.app_ver):
                print("version.json says no newer version")
                return False
            ...
            if expected_sha and real_sha.lower() != expected_sha:
                ...
                print("sha256 mismatch, abort update")
                return False
            # 備份舊版本
            self.backup()
        ...
        # 到這裡表示更新成功
        print("update success, please restart app")
        return True